...
if (isset($_REQUEST['draw'])) {
    ...
    $di_sql = "SELECT * FROM `tbl_location` WHERE is_deleted= 0";
    ...
    while ($row = mysqli_fetch_assoc($diRecords)) {
        ...
        $row['action'] .= '<a href="master-location?e_id=' . $row['id'] . '" type="button" class="btn btn-sm btn-info btn-table" ><i class="fa fa-edit me-1"></i>Edit</a>';
...
if (isset($_POST['submit'])) {
    ...
    $location         = filtervar($mysqli, $_POST['location']);
    ...
    if ($form_action == 'ADD') {
        ...
        $query = "INSERT INTO `tbl_location` SET $data";
        $id    = $mysqli->insert_id;
        $msg   = "Successfully Inserted";
    } elseif ($form_action == 'UPDATE') {
        $data .= "`updated_by` = '$user_id',`updated_at` = '$gen_date'";
        $query = "UPDATE `tbl_location` SET $data WHERE `id`='$id'";
        $msg   = "Successfully Updated";
    }

    if ($mysqli->query($query)) {
        $result = array('result' => true, 'redirect' => 'master-location', 'dhSession' => ["success" => $msg]);
    } else {
...
if (isset($_REQUEST['e_id'])) {
    ...
    $get_result = $mysqli->query("SELECT * FROM `tbl_location` WHERE `id`='$id' ");
                                ...
                                <form class="dhForm" method="post" autocomplete="off">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <input type="text" class="form-control " name="location" id="location" value="<?php echo (isset($row['location'])) ? $row['location'] : '' ?>" placeholder="Name" required>
                                        </div>
        ...
        <script>
            ...
            function delete_row(id) {
                $.dhConfirm({
                    ...
                    dhUrl: "master-location?delete&id=" + id
                })
            }

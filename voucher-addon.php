<?php
    ...
    if(isset($_REQUEST['draw'])){
       ...
       $searchValue     = mysqli_real_escape_string($mysqli, $_POST['search']['value']); // Search value
        ...
        $di_sql = "SELECT * FROM `tbl_voucher_addon` WHERE `is_deleted`=0";
...
//   </ul>
// </div>
            $row['addon_name']=getAddonName($row['addon_id']);
    ...
    if(isset($_REQUEST['delete']) && !empty($_REQUEST['id'])){
        ...
        $update_query = $mysqli->query("UPDATE `tbl_voucher_addon` SET `is_deleted`=1 WHERE `id`='$id'");
        if($update_query){
    ...
    <script>
        ...
        function delete_row(id){
          $.dhConfirm({
            ...
            dhUrl: "voucher-addon?delete&id=" + id
          })
        }

...
if (isset($_REQUEST['draw'])) {
    ...
    $di_sql = "SELECT * FROM `tbl_master_meals` WHERE `is_deleted`=0";

    ## Search
    $searchQuery = " ";
    if ($searchValue != '') {
        $di_sql .= " AND (`meal_plan` LIKE '%" . $searchValue . "%')";
    }
    ...
    while ($row = mysqli_fetch_assoc($diRecords)) {
        ...
        $row['action'] .= '<a href="master-meal-plan?e_id=' . $row['id'] . '" type="button" class="btn btn-sm btn-info btn-table" ><i class="fa fa-edit me-1"></i>Edit</a>';
...
if (isset($_POST['submit'])) {
    ...
    $meal_plan         = filtervar($mysqli, $_POST['meal_plan']);
    ...
    if ($form_action == 'ADD') {
        ...
        $query = "INSERT INTO `tbl_master_meals` SET $data";
        $id    = $mysqli->insert_id;
        $msg   = "Successfully Inserted";
    } elseif ($form_action == 'UPDATE') {
        $data .= "`updated_by` = '$user_id',`updated_at` = '$gen_date'";
        $query = "UPDATE `tbl_master_meals` SET $data WHERE `id`='$id'";
        $msg   = "Successfully Updated";
    }
    if ($mysqli->query($query)) {
        $result = array('result' => true, 'redirect' => 'master-meal-plan', 'dhSession' => ["success" => $msg]);
    } else {
...
if (isset($_REQUEST['delete']) && !empty($_REQUEST['id'])) {
    ...
    $update_query = $mysqli->query("UPDATE `tbl_master_meals` SET `is_deleted`=1 WHERE `id`='$id'");
    if ($update_query) {
...
if (isset($_REQUEST['e_id'])) {
    ...
    $get_result = $mysqli->query("SELECT * FROM `tbl_master_meals` WHERE `id`='$id' ");
                                ...
                                <form class="dhForm" method="post" autocomplete="off">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <input type="text" class="form-control " name="meal_plan" id="meal_plan" value="<?php echo (isset($row['meal_plan'])) ? $row['meal_plan'] : '' ?>" placeholder="Name" required>
                                        </div>
        ...
        <script>
            ...
            function delete_row(id) {
                $.dhConfirm({
                    ...
                    dhUrl: "master-meal-plan?delete&id=" + id
                })
            }
